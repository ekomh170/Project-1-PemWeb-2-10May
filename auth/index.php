	...
	<head>
		<title>Nakes Login | Eko Muchamad Haryono</title>
		<meta charset="UTF-8">
	...
	if (isset($_POST['submit'])) {
		...
		$user = $dbh->prepare("SELECT * FROM users WHERE username = ? AND password = ?");
		$user->execute([
		...
		<script>
			function autoFill() {
				if (document.getElementById('ckb1').checked) {
					document.getElementById('username').value = 'ekomh';
